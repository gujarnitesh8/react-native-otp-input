...
export default class OTPInputView extends Component {
    constructor(props) {
        ...
        this.renderOneInputField = (_, index) => {
            const { codeInputFieldStyle, codeInputHighlightStyle, secureTextEntry, editable, keyboardType, selectionColor, filledInputStyle, keyboardAppearance } = this.props;
            ...
            return (<View pointerEvents="none" key={index + "view"} testID="inputSlotView">
                <TextInput testID="textInput" underlineColorAndroid='rgba(0,0,0,0)' style={digits[index] ? [defaultTextFieldStyle, filledInputStyle] : selectedIndex === index ? [defaultTextFieldStyle, codeInputFieldStyle, codeInputHighlightStyle] : [defaultTextFieldStyle, codeInputFieldStyle]} ref={ref => { this.fields[index] = ref; }} onChangeText={text => {
                    this.handleChangeText(index, text);
